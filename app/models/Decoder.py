...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, encoder_output, encoder_mask, decoder_mask):
        ...
        x = self.res_connection[0](x, lambda x: self.self_attn_block(x,x,x, decoder_mask))
        x = self.res_connection[1](x, lambda x: self.cross_attn_block(x,encoder_output,encoder_output, encoder_mask))
